...
public class Kedou {
	private static final String path = "F://vs";

	private static String url = "http://www.cao0001.com";
	...
	public static List<String> getIndexVideoLinks(Html html) {
		List<String> all = html.xpath("//div[@id='list_videos_most_recent_videos']").links()
				.regex("http://www\\.cao0003\\.com/videos/.*").all();
		System.out.print(all);
		return all;
	}

	public static String getNextPage(Html html) {
		String page = html.xpath("//div[@id='list_videos_most_recent_videos']/div/div/ul/li[@class='next']").links()
				.regex("http://www\\.cao0003\\.com/latest.*").get();
		return page;
	}
